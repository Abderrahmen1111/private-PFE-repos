...
const products: Product[] = [
  { id: 1, image: "💻", tag: "Idéal pour études & usage quotidien", tagIcon: "🎓", tagColor: "from-violet-50 to-indigo-50 text-indigo-700 border-indigo-100", title: "Lenovo IdeaPad 5 – 15.6″ FHD, Ryzen 5, 8Go RAM", price: "2 400 TND", seller: "TechStore Sfax", sellerRating: 5, sellerReviews: 128, distance: "1.2 km", availability: "Disponible en magasin", delivery: true, badges: ["Achat vérifié", "Garantie 2 ans"], gradient: "from-indigo-400/20 to-violet-400/20" },
  { id: 2, image: "🖥️", tag: "Choix équilibré performance / prix", tagIcon: "💎", tagColor: "from-emerald-50 to-teal-50 text-teal-700 border-teal-100", title: "HP Pavilion 15 – Core i5 12ème Gén, 16Go, SSD 512Go", price: "3 100 TND", seller: "Informatique Plus", sellerRating: 4, sellerReviews: 87, distance: "3.5 km", availability: "Stock limité", delivery: true, badges: ["Commerçant vérifié"], gradient: "from-teal-400/20 to-emerald-400/20" },
  { id: 3, image: "⚡", tag: "Haute performance pour logiciels exigeants", tagIcon: "🚀", tagColor: "from-amber-50 to-orange-50 text-orange-700 border-orange-100", title: "ASUS ROG Zephyrus G14 – Ryzen 9, RTX 4060, 32Go", price: "5 800 TND", seller: "GameZone Tunis", sellerRating: 5, sellerReviews: 214, distance: "8.1 km", availability: "Disponible en ligne", delivery: true, badges: ["Achat vérifié", "Commerçant vérifié", "Garantie 2 ans"], gradient: "from-orange-400/20 to-amber-400/20" },
  { id: 4, image: "🎯", tag: "Budget maîtrisé, qualité assurée", tagIcon: "💰", tagColor: "from-sky-50 to-blue-50 text-blue-700 border-blue-100", title: "Acer Aspire 3 – Core i3, 8Go, SSD 256Go, 15.6″", price: "1 650 TND", seller: "ElectroMart Sfax", sellerRating: 4, sellerReviews: 63, distance: "0.8 km", availability: "Disponible en magasin", delivery: false, badges: ["Garantie 1 an"], gradient: "from-blue-400/20 to-sky-400/20" },
  { id: 5, image: "🍎", tag: "Écosystème premium & longévité exceptionnelle", tagIcon: "✨", tagColor: "from-rose-50 to-pink-50 text-rose-700 border-rose-100", title: "MacBook Air M2 – 8Go, SSD 256Go, 13.6″ Liquid Retina", price: "4 200 TND", seller: "Apple Reseller Tunis", sellerRating: 5, sellerReviews: 341, distance: "12 km", availability: "Disponible en ligne", delivery: true, badges: ["Achat vérifié", "Commerçant vérifié"], gradient: "from-pink-400/20 to-rose-400/20" },
  { id: 6, image: "🔥", tag: "Rapport qualité/prix imbattable", tagIcon: "💎", tagColor: "from-purple-50 to-violet-50 text-purple-700 border-purple-100", title: "Dell Inspiron 15 – Core i5, 8Go, 512Go SSD, FHD IPS", price: "2 750 TND", seller: "Dell Official Sfax", sellerRating: 5, sellerReviews: 156, distance: "2.3 km", availability: "Disponible en magasin", delivery: true, badges: ["Achat vérifié", "Garantie 2 ans"], gradient: "from-violet-400/20 to-purple-400/20" },
];

const filters = [
  { label: "Usage étudiant", icon: "🎓" }, { label: "Qualité/prix", icon: "💎" },
  { label: "Haute performance", icon: "🚀" }, { label: "Budget maîtrisé", icon: "💰" },
  { label: "Prix", icon: "" }, { label: "Localisation", icon: "" },
  { label: "Disponibilité", icon: "" }, { label: "Livraison", icon: "" }, { label: "Note", icon: "⭐" },
];

const sortOptions = ["Pertinence", "Prix croissant", "Les mieux notés ⭐", "Proches de vous 📍"];
...
const similarSearches = [
  "PC portable gaming étudiant", "Meilleur ordinateur portable 2024",
  "Laptop léger longue autonomie", "PC pour développeur", "Ordinateur étudiant médecine",
];
...
// ─── Product Card ─────────────────────────────────────────────────────────────
function ProductCard({ p, compared, onCompare, onViewDetails }: {
  p: Product; compared: boolean; onCompare: () => void; onViewDetails: () => void;
}) {
  ...
  return (
    <div
      ...
      className="group relative bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-500 overflow-hidden border border-stone-100 hover:border-stone-200 hover:-translate-y-1 cursor-pointer"
    >
      ...
      <div className="p-4 flex flex-col gap-2.5">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {p.delivery && <span className="inline-flex items-center gap-1 text-[11px] text-sky-700 bg-sky-50 px-2 py-0.5 rounded-full">🚚 Livraison possible</span>}
          <span className="inline-flex items-center gap-1 text-[11px] text-stone-500 bg-stone-50 px-2 py-0.5 rounded-full">📍 À {p.distance}</span>
        </div>
        <div className="flex flex-wrap gap-1">
          {p.badges.map(b => (
            <span key={b} className="text-[10px] text-violet-600 bg-violet-50 border border-violet-100 px-2 py-0.5 rounded-full font-medium">✓ {b}</span>
          ))}
        </div>
        <div className="flex gap-2 pt-1">
          <button
            onClick={e => { e.stopPropagation(); onViewDetails(); }}
            className="flex-1 text-[13px] font-semibold bg-stone-900 text-white py-2 rounded-xl hover:bg-stone-700 transition-colors duration-200"
          >
            Voir détails
          </button>
          <button
            onClick={e => e.stopPropagation()}
            className="flex-1 text-[13px] font-medium border border-stone-200 text-stone-700 py-2 rounded-xl hover:bg-stone-50 transition-colors duration-200"
          >
            🛒 Panier
          </button>
          <button
  ...
  return (
    <div className="min-h-screen bg-[#F8F7F5]" style={{ fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      <style>{`
        ...
        .card-delay-1 { animation-delay: 0.05s; } .card-delay-2 { animation-delay: 0.1s; }
        .card-delay-3 { animation-delay: 0.15s; } .card-delay-4 { animation-delay: 0.2s; }
        .card-delay-5 { animation-delay: 0.25s; } .card-delay-6 { animation-delay: 0.3s; }
      `}</style>
      ...
      <nav className="bg-white border-b border-stone-100 px-6 py-3.5 flex items-center justify-center sticky top-0 z-50">
        <div className="flex items-center gap-3 bg-stone-100 rounded-xl px-4 py-2 w-80">
          ...
          <span className="text-stone-500 text-sm">PC idéal pour étudiant</span>
        </div>
      </nav>
      ...
      <div className="bg-white border-b border-stone-100 px-6 py-4 fade-up">
        <div className="max-w-6xl mx-auto flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-violet-50 flex items-center justify-center flex-shrink-0">
            <svg style={{width:"18px",height:"18px"}} className="text-violet-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
            </svg>
          </div>
          <div>
      ...
      <div className="sticky top-[57px] z-40 bg-white/90 backdrop-blur-md border-b border-stone-100 shadow-sm">
        <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between gap-4">
          <div ref={filterRef} className="flex items-center gap-2 overflow-x-auto hide-scroll flex-1 pb-0.5">
            {filters.map(f => {
              const active = activeFilters.includes(f.label);
              return (
                <button key={f.label} onClick={() => toggleFilter(f.label)}
                  className={`flex-shrink-0 flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-sm font-medium border transition-all duration-200 shadow-sm ${active ? "bg-stone-900 text-white border-stone-900 shadow-md" : "bg-white text-stone-600 border-stone-200 hover:border-stone-400 hover:text-stone-900"}`}>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <select value={sortBy} onChange={e => setSortBy(e.target.value)}
              className="text-sm font-medium text-stone-700 bg-stone-50 border border-stone-200 rounded-xl px-3 py-1.5 cursor-pointer focus:outline-none appearance-none pr-7"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {products.slice(0, 3).map((p, i) => (
            <div key={p.id} className={`fade-up card-delay-${i + 1} opacity-0`}>
              <ProductCard p={p} compared={compared.includes(p.id)} onCompare={() => toggleCompare(p.id)} onViewDetails={() => handleViewDetails(p.id)} />
            </div>
          ))}
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-stone-50">
            {recommendations.map(r => (
              <button key={r.label} className="flex items-center gap-3 px-6 py-4 hover:bg-stone-50 transition-colors text-left group">
                <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${r.color} flex items-center justify-center text-lg shadow-sm group-hover:scale-105 transition-transform duration-200`}>{r.icon}</div>
                <div><p className="text-sm font-semibold text-stone-700">{r.label}</p><p className="text-xs text-stone-400">{r.sub}</p></div>
        ...
        <div className="mt-12 grid sm:grid-cols-2 gap-5">
          <div className="bg-white rounded-2xl border border-stone-100 p-5 shadow-sm">
            ...
            <div className="space-y-3">
              {products.slice(0, 3).map((p, i) => (
                <div key={p.id} onClick={() => handleViewDetails(p.id)} className="flex items-center gap-3 group cursor-pointer">
          ...
          <div className="bg-white rounded-2xl border border-stone-100 p-5 shadow-sm">
            ...
            <div className="flex flex-wrap gap-2">
              {similarSearches.map(s => (
                <button key={s} className="text-sm text-stone-600 bg-stone-50 border border-stone-200 px-3 py-1.5 rounded-xl hover:bg-stone-900 hover:text-white hover:border-stone-900 transition-all duration-200">{s}</button>
              ))}
            </div>
          </div>
        </div>
      ...
      {compared.length > 0 && (
        <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 bg-stone-900 text-white rounded-2xl px-5 py-3 shadow-2xl flex items-center gap-4 fade-up">
          ...
          <button className="bg-white text-stone-900 text-sm font-semibold px-4 py-1.5 rounded-xl hover:bg-stone-100 transition-colors">Comparer ⚖️</button>
