...
export function ReservationSummary({ data, fee, currency }: ReservationSummaryProps) {
  const formatDate = (d: Date) =>
    `${d.getDate()} ${MONTH_SHORT[d.getMonth()]} ${d.getFullYear()}`;

  const total = fee * data.guests;
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50 overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-200 flex items-center gap-2">
        ...
        <span className="text-sm font-semibold text-slate-800">Reservation Summary</span>
      </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="border-t border-slate-200 pt-3 space-y-1.5">
          <div className="flex justify-between text-sm">
            <span className="text-slate-500">Fee per person</span>
            <span className="text-slate-700">{currency}{fee.toFixed(2)}</span>
          </div>
